...
#Все данные в сорсовом файле рассматриваются как одна новость (думаю что их можно прочитать и добавить как несколько новостей с отдельным заголовком для каждой)
...
def ReadSportFile():
    fromfile = open('sport_source.txt').read()
    return fromfile
#Читаем текст обычной новости из файла:
def ReadNewsFile():
    fromfile = open('news_source.txt').read()
    return fromfile
...
def AddNews():
    ...
    if inputtype == '1':
        ...
        newstype = input("Выберете тип новости (1 - Cпорт, 2 - Hовость):\n")
        if newstype == '1':
            createContent(ReadSportFile(), '\n===SPORT_NEWS===')
            Publish()
        elif newstype == '2':
            createContent(ReadNewsFile(), '\n===NEWS===')
            Publish()
